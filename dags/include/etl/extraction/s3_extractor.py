...
class Sdrr:
    def __init__(
        ...
        self.context = context
        self.execution_date = context.get("ds")
        self.s3_src_hook = s3_src_hook or S3Hook(aws_conn_id="aws_airflow_dest_user")
        self.s3_dest_hook = s3_dest_hook or S3Hook(aws_conn_id="aws_airflow_dest_user")
    ...
    def copy_web_media_complaints(self) -> Dict[str, any]:
        """Copies social media complaints from source S3 to destination S3."""
        return self.copy_data(
            src_key=f"staging/coretelecoms-web-complaints/web-complaints-{self.execution_date}.parquet",
            dest_prefix=config.WEB_COMPLAINTS_STAGING_DEST,
            obj_prefix=config.WEB_COMPLAINTS_OBJ_PREFIX,
            obj_type="web complaints",
        )
